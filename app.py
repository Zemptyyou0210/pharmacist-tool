...
st.markdown("""
<style>
div[data-testid="stChipGroupChip"] { font-size: 13px; }
...
def load_drug_db():
    try:
        ...
        for _, row in df.iterrows():
            ...
            atc  = str(row[atc_col]).strip().upper()[:1]
            if name and atc:
                records.append({"name": name, "atc": atc})
        return records
    except Exception:
        return []
...
ATC_MAP = {
    "A": "A-消化/代謝", "B": "B-血液", "C": "C-心血管",
    "D": "D-皮膚",     "G": "G-泌尿生殖", "H": "H-荷爾蒙",
    "J": "J-抗感染",   "L": "L-腫瘤",  "M": "M-肌肉骨骼",
    "N": "N-神經",     "P": "P-寄生蟲", "R": "R-呼吸",
    "S": "S-感覺",     "V": "V-其他",   "": "未分類"
}
...
# ── 比對院內品項（部分符合）────────────────────────────────
def lookup_atc(name: str) -> tuple[str, str]:
    """回傳 (atc_code, matched_name)"""
    u = name.upper().replace(r"[^A-Z0-9]", " ").strip()
...
# ── 藥品切割────────────────────────────────────────────────
def parse_drug_text(raw: str) -> list[dict]:
    drugs = []
    stopped_set = set()
    # 偵測住院停用
    ...
    for n in stopped_set:
        if len(n) > 2:
            ...
            drugs.append({"name": n.title(), "status": "住院停用",
                          "atc": atc, "matched": matched,
                          "dose": "", "freq": "", "route": "", "note": ""})
    for n in extract(after):
        atc, matched = lookup_atc(n)
        drugs.append({"name": n, "status": "自備",
                      "atc": atc, "matched": matched,
                      "dose": "", "freq": "", "route": "", "note": ""})
    for n in extract(before):
        if not any(s in n.upper() for s in stopped_set):
            atc, matched = lookup_atc(n)
            drugs.append({"name": n, "status": "正常服用",
                          "atc": atc, "matched": matched,
                          "dose": "", "freq": "", "route": "", "note": ""})
    return drugs
...
def build_excel(tab, drugs, vitals, free_note, way, target, edu_or_suggest, ref, result):
    ...
    for d in drugs:
        rows.append({
            ...
            "用藥狀態": d["status"],
            "ATC": d["atc"],
            "ATC說明": ATC_MAP.get(d["atc"], ""),
            "劑量": d["dose"],
            "頻次": d["freq"],
            "給藥途徑": d["route"],
            "藥品備註": d["note"],
            "院內對照品項": d.get("matched", ""),
        })
    df_drug = pd.DataFrame(rows) if rows else pd.DataFrame(columns=["Tab","藥品名稱","用藥狀態","ATC","ATC說明","劑量","頻次","給藥途徑","藥品備註","院內對照品項"])
    meta = {
        "項目": ["諮詢/建議方式","指導/建議對象","衛教/建議內容","參考資料","結果","備註",
                  "追蹤日期","BP","GluAC","GluPC","HbA1c","Cr"],
        "內容": [
            ",".join(way), ",".join(target), ",".join(edu_or_suggest),
            ",".join(ref), result, free_note,
            ",".join(v["date"] for v in vitals if v["date"]),
            ",".join(v["bp"]   for v in vitals if v["bp"]),
            ",".join(v["gluAC"]for v in vitals if v["gluAC"]),
            ",".join(v["gluPC"]for v in vitals if v["gluPC"]),
            ",".join(v["hba1c"]for v in vitals if v["hba1c"]),
            ",".join(v["cr"]   for v in vitals if v["cr"]),
        ]
    }
...
if db_count:
    st.caption(f"✅ 院內品項已載入：{db_count} 筆（drug_db.csv）")
else:
    st.caption("⚠️ 未找到 drug_db.csv，使用內建對照表。將 drug_db.csv 放在同資料夾即可自動載入。")
...
def drug_section(key_prefix):
    ...
    with st.expander("📋 貼上藥品文字 → 自動切割", expanded=True):
        paste = st.text_area("貼上原始文字", height=80, key=f"{key_prefix}_paste",
            placeholder="住院中未服用Oxybutinin  其餘自備  AMLODIPINE Besylate  Famotidine  tenofovir25mg  Zolpidem")
...
with tab1:
    ...
    st.subheader("自由備註")
    free1 = st.text_area("", max_chars=200, height=80, key="free1",
        placeholder="其他說明…（200字以內）")
    ...
    if st.button("⚡ 產生結構化文字", type="primary", key="gen1"):
        ...
        char_count = len(short.replace("\n",""))
        st.session_state["out1"] = short
        st.session_state["out1_len"] = char_count

    if "out1" in st.session_state:
        out = st.session_state["out1"]
        n   = st.session_state["out1_len"]
        color = "🔴" if n > 200 else "🟡" if n > 160 else "🟢"
        st.markdown(f"**HIS 備註欄輸出** {color} {n}/200 字")
        st.code(out, language=None)
        if n > 200:
            st.warning("⚠️ 超過 200 字！請刪減衛教項目或縮短備註。")

        excel_data = build_excel("諮詢紀錄", drugs1, vitals1, free1, way, target, edu, ref, result)
        st.download_button("📥 下載完整 Excel", excel_data,
                           file_name="諮詢紀錄.xlsx",
...
with tab2:
    ...
    st.subheader("自由備註")
    free2 = st.text_area("", max_chars=200, height=80, key="free2",
        placeholder="其他說明…（200字以內）")
    ...
    if st.button("⚡ 產生結構化文字", type="primary", key="gen2"):
        ...
        n2 = len(short2.replace("\n",""))
        st.session_state["out2"] = short2
        st.session_state["out2_len"] = n2

    if "out2" in st.session_state:
        out2 = st.session_state["out2"]
        ...
        excel_data2 = build_excel("藥事建議", drugs2, vitals2, free2, sway, starget, suggest, sref, sresult)
        st.download_button("📥 下載完整 Excel", excel_data2,
                           file_name="藥事建議.xlsx",
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

# ── 分析工具 ────────────────────────────────────────────
with st.expander("🔬 貼入 HIS 紀錄 → 解析結構"):
    raw = st.text_area("貼入原始紀錄", height=120, key="parse_raw",
        placeholder="[方式:電話諮詢]\n[藥品-自備:Warfarin[B](5mg/QD/PO)]\n[追蹤(3/2):BP:141,GluAC:140]")
    if st.button("🔍 解析", key="do_parse"):
        ...
        if matches:
            ...
            st.download_button("📥 下載 CSV", df.to_csv(index=False, encoding="utf-8-sig"),
                               "parsed.csv", "text/csv")
        else:
